...
import {
  ...
  Clock, BedDouble, BarChart3, Trash2, X, Thermometer,
} from "lucide-react";
import {
...
export default function SleepTracker() {
  ...
  const startSleep = async () => {
    ...
    try { localStorage.setItem(ACTIVE_KEY, JSON.stringify(session)); }
    catch (e) { console.error(e); }
    setView("home");
  };
    ...
    <div className="min-h-screen w-full font-body relative overflow-hidden" style={styles.root}>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-5 py-6 md:px-8 md:py-10">
        ...
        {loading ? (
          ...
          <History entries={entries} onDelete={deleteEntry} onClear={clearAll} />
        ) : (
...
/* ============ History ============ */
function History({ entries, onDelete, onClear }) {
  if (entries.length === 0) return <EmptyState message="no sleep logs yet." />;
  const sorted = [...entries].sort((a, b) => b.endedAt - a.endedAt);

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div className="text-sm text-[color:var(--muted)]">{entries.length} night{entries.length === 1 ? "" : "s"} logged</div>
        <button onClick={onClear} className="text-xs flex items-center gap-1 px-3 py-2 rounded-full transition-colors hover:text-[color:var(--warn)]" style={{ color: "var(--muted)" }}>
        ...
        {sorted.map((e) => {
          ...
          return (
            <div key={e.id} className="rounded-xl p-4 flex items-center gap-4" style={{ background: "var(--panel)", border: "1px solid var(--line)" }}>
              ...
              <button onClick={() => onDelete(e.id)} className="p-2 rounded-full transition-colors hover:bg-[color:var(--line)]" style={{ color: "var(--muted)" }}>
                <X size={14} />
              </button>
            </div>
          );
